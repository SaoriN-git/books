    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
      <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6 text-gray-900">
          ...
          <form class="w-full" action="{{ route('book.update', ['id' => $book->id]) }}" method="POST">
            @csrf
            @method('PATCH')
            ...
            <div class="bg-white shadow overflow-hidden sm:rounded-lg">
              ...
              <div class="border-t border-gray-200">
                <dl>
                    ...
                    <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                      <select id="status" name="status" autocomplete="country-name"
                        ...
                        @foreach (App\Models\Book::BOOK_STATUS_OBJECT as $key => $value)
                          <option value="{{ $key }}" @if ($key === $book->status) selected @endif>
                            {{ $value }}</option>
                        @endforeach
                      </select>
                    </dd>
                  ...
                  <div class="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                    <dt class="text-sm font-medium text-gray-500">読み終わた日</dt>
                    <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                      <div class="col-span-6 sm:col-span-3">
                        <input type="text" name="read_at" id="read_at" value="{{ $book->read_at }}"
                          placeholder="例) 2022-05-01" autocomplete="given-name"
                          class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                      </div>
                    </dd>
                  </div>
                    ...
                    <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                      <div class="mt-1">
                        <textarea id="note" name="note" rows="5"
                          class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 mt-1 block w-full sm:text-sm border border-gray-300 rounded-md">{{ $book->note }}</textarea>
                      </div>
                    </dd>
                  </div>
                </dl>
